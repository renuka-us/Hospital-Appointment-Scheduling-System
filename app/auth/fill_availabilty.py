...
def appt_history():
    ...
    try:
        ...
        cursor = mysql.connection.cursor()
        check_availability_query = "SELECT * FROM doctor_availability WHERE Doc_ID=%s AND Date_Available=%s AND start_time=%s AND end_time=%s"
        cursor.execute(check_availability_query, (doctor_id, date, start_time, end_time))
...
def get_appointment_history():
    ...
    try:
        ...
        if role_id!="2":
            ...
            cursor.execute("SET SQL_SAFE_UPDATES = 0;")
            # cursor.execute("UPDATE Appointment SET status= 'completed' WHERE appt_date <= %s and end_time <= %s;", (current_datetime,current_time))
            
            cursor.execute(
                " UPDATE Appointment SET status= 'completed' WHERE appt_date <= curdate() and end_time <= current_time();")
        ...
        # Check if appointment_history is empty
        if not appointment_history:
            # Return a custom message if no appointments were found
            return jsonify({'message': 'No appointments found for the given patient.'}), 404
...
@fill_availability_bp.route('/search', methods=['GET'])
@jwt_required()
def search():
    print("search")
    try:
        ...
        search_value = request.args.get(search_type)
        ...
        if search_type == 'date_selected':
            cursor.execute('''
                           SELECT users.Name, doctor_availability.Speciality, doctor_availability.Date_Available, doctor_availability.start_time, doctor_availability.end_time
                           FROM users INNER JOIN doctor_availability
                           WHERE doctor_availability.Date_Available= %s AND users.User_ID=doctor_availability.Doc_ID;''', (search_value, ))
        elif search_type == 'speciality_selected':
            cursor.execute('''
                           SELECT users.Name, doctor_availability.Speciality, doctor_availability.Date_Available, doctor_availability.start_time, doctor_availability.end_time
                           FROM users INNER JOIN doctor_availability
                           WHERE doctor_availability.Speciality= %s AND users.User_ID=doctor_availability.Doc_ID;''', (search_value, ))
        else:
            return jsonify({"error": "Invalid search type. Please provide 'type' query parameter as 'date_selected' or 'speciality_selected'."}), 400
        ...
        return jsonify({'search_results': search_data}), 200
 
    except Exception as e:
        return jsonify({"error": str(e)}), 500
   
 
@fill_availability_bp.route('/check_availability', methods=['POST'])
def check_availability():
    ...
    try:
        ...
        query = ''' SELECT * FROM doctor_availability WHERE Doc_ID = %s AND Date_Available = %s AND start_time = %s AND end_time = %s AND speciality=%s '''
        cursor.execute(query, (user_id, date, start_time, end_time, specialty))
        ...
        slot_is_available = len(search_results)
 
        print(slot_is_available)
 
        if slot_is_available==0:
            # Return a 200 status code with a JSON response indicating the slot is available
            return (jsonify({'isAvailable': True}), 200)
        else:
            # Return a 404 status code with a JSON response indicating the slot is not available
            return (jsonify({'isAvailable': False}), 404)
